...
> 状态:M1–M3 已交付并通过验收;M3.5 项目隔离设计已定稿(实现分批推进);M4 为可选项,未开工。
...
**范围**:projects 表 + branches 复合主键((project, name))、库 schema 版本与对象编码版本解耦、v1→v2 自动迁移、repo/CLI 项目作用域(KB_PROJECT / -p)、短标识项目内解析。

**验收标准**
- v1 存量库打开时自动迁移:存量分支落入 default 项目且可读、地址不变,库版本升至 2(已由迁移测试覆盖)
...
**状态**:设计定稿(schema.sql v2 + 本节 + DESIGN 同步 + 迁移与兼容测试已落地);repo/CLI 项目作用域为下一步实现。
