...
func (p *PG) BranchDescribe(ctx context.Context, project, name, description string) error {
	tag, err := p.pool.Exec(ctx,
		"UPDATE branches SET description = $3 WHERE project = $1 AND name = $2", project, name, description)
	if err != nil {
...
func (p *PG) BranchList(ctx context.Context, project string) ([]BranchRef, error) {
	rows, err := p.pool.Query(ctx,
		"SELECT name, addr, description FROM branches WHERE project = $1 ORDER BY name", project)
	if err != nil {
