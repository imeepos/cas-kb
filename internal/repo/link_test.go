...
func TestM2_LinkCrossVersionSelfConsistent(t *testing.T) {
	...
	eOld, ok := treeOld.Lookup("a")
	if !ok || eOld.Addr != addrA1 {
		t.Fatalf("旧快照应解析到旧 A:%v != %v", eOld.Addr, addrA1)
	}
	eNew, ok := treeNew.Lookup("a")
	if !ok || eNew.Addr != addrA2 {
		t.Fatalf("新快照应解析到新 A:%v != %v", eNew.Addr, addrA2)
	}
	// B 在两个快照中地址一致(未变),其链接指向的 slug 在两个快照中分别解析到新旧 A
	bOld, _ := treeOld.Lookup("b")
	bNew, _ := treeNew.Lookup("b")
	if bOld != bNew {
...
func TestM2_RemoveSlugAffectsOnlyNewSnapshot(t *testing.T) {
	...
	if _, ok := tree1.Lookup("a"); !ok {
		t.Fatal("旧快照应仍含 a")
	}
	if _, ok := tree2.Lookup("a"); !ok {
		t.Fatal("删除前快照应仍含 a")
	}
	if _, ok := tree3.Lookup("a"); ok {
		t.Fatal("新快照不应含 a")
	}
	if _, ok := tree3.Lookup("b"); !ok {
		t.Fatal("新快照应仍含 b")
	}
}
