...
// Ollama 适配器端点与字段核实结论(2026-09-03 核对官方文档
// github.com/ollama/ollama docs/api.md(main 分支,raw.githubusercontent.com
// 实取原文),「Generate Embeddings」节,逐字段核对):
//   - 端点:POST /api/embed(注意不是旧版单数 /api/embeddings——旧端点
//     只收 prompt 单串,文档已标注 superseded,勿混用);
//   - 请求字段:model(模型名)、input(单个字符串或字符串数组);
//   - 响应字段:embeddings 为「数组的数组」——input 传数组时,每个输入串
//     恰好对应一个向量数组,顺序与输入一致(文档 Multiple input 示例钉死:
//     两条输入 → embeddings 两行,且逐行与输入同序);
