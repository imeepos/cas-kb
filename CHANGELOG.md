...
- 流程制度化批次(T60,零产品代码;依据 docs/research/ops-patterns.md T57 实施清单):①`scripts/session-watch.sh` 会话心跳巡检——`git for-each-ref refs/heads` 算会话分支最近 commit 年龄(排除 main 与当前分支),>90min(`STALE_MIN` 可调)输出 `STALE <分支> <年龄>min` 且退出 1,健康逐分支输出年龄并退出 0;判定归脚本、处置归负责人(不删分支不合并);AGENTS.md 增「会话心跳纪律」一节(三阈值:心跳 ≤30min/死亡 >90min/墙钟 6h;复活三查:查已交付/查分支关系/查撞车,复用优先于重写;冻结名单;复活首 commit 注明「复活自 Txx,已核对 main@<sha> 无重复实现」)。②`scripts/regression.sh` 季度/发版前回归聚合器——依序 verify → drill-multi → drill-serve,逐段 `ok/not ok <段名>`,任一段非零整体退出非零;KB_BIN 一次构建两 drill 复用;全量输出 tee 到 `/tmp/regression-<时间戳>.log`(不入 git)。③launchd 季度任务 `docs/launchd/com.caskb.regression.plist`(1/4/7/10 月 1 日 02:00;ProgramArguments 数组式直指脚本、不经 sh -c——规避 T43 D1;交付前 `plutil -lint` 通过)。④README「开发与测试」补回归用法与频率、归档边界:报告(结论)入 `docs/review/`(入 git)、日志(现场)入 `/tmp`(不入 git)。⑤DESIGN §8.5 `/api/v1/log` 行补注记「合并快照 `parents=[ours,theirs]` 与 merge-state 的 `ours`/`theirs` 可交叉验证」,drill-multi leg1/leg2 各增「双亲含 ours 头」对称断言(原只验 theirs;契约与文本分隔符均不动)
