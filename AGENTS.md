...
本工作区是 **cas-kb 知识库系统**:交付设计文档与 Go 实现(CLI `kb`)。ROADMAP 的 M1–M3.8 已交付(含 M3.5 项目隔离、M3.6 回退与历史读取、M3.7 AI 选用元数据/schema v3、M3.8 目录层级/schema v4:tree 条目带类型,目录可嵌套,条目按全路径定位);M4(检索/HTTP API)为可选项,未开工。
